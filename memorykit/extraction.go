...
func (c ExtractionWorkerConfig) Validate() error {
	...
	if nilInterface(c.Jobs) || nilInterface(c.Memories) || nilInterface(c.Reader) ||
		nilInterface(c.Extractor) || nilInterface(c.ValidateContent) || c.NewCandidateID == nil || c.Now == nil ||
		strings.TrimSpace(c.WorkerID) == "" || c.WorkerID != strings.TrimSpace(c.WorkerID) ||
		!validMetadata(c.WorkerID, c.Limits) || c.LeaseDuration <= 0 || c.MaxAttempts <= 0 {
...
func NewExtractionWorker(config ExtractionWorkerConfig) (*ExtractionWorker, error) {
	...
	return &ExtractionWorker{
		...
		maxAttempts: config.MaxAttempts, newCandidateID: config.NewCandidateID, now: config.Now,
	}, nil
}
...
func (w *ExtractionWorker) RunOnce(ctx context.Context) (bool, error) {
	...
	for index, draft := range drafts {
		...
		candidateID := w.newCandidateID(job.ID, index)
		if err := ValidateMemoryID(candidateID); err != nil {
			return true, w.fail(ctx, job, "candidate_validate_failed", err, claimNow)
		}
		_, err := w.memories.Create(ctx, CreateRequest{
		...
		if err = contextError(ctx, err); err != nil {
			return true, w.fail(ctx, job, "candidate_write_failed", err, claimNow)
		}
	}

	completeNow, clockErr := w.transitionTime(claimNow)
	if clockErr != nil {
		return true, w.failAt(ctx, job, "candidate_write_failed", clockErr, claimNow)
	}
	err = w.jobs.CompleteExtraction(ctx, job.ID, w.workerID, completeNow)
	if err = contextError(ctx, err); err != nil {
...
func (w *ExtractionWorker) failAt(
	...
	transitionErr := w.jobs.FailExtraction(ctx, job.ID, w.workerID, code, w.maxAttempts, transitionNow)
...
func ProjectTrusted(ctx context.Context, store LifecycleStore, validator ContentValidator, projection TrustedProjection) (Memory, error) {
	...
	return created, contextError(ctx, err)
}
